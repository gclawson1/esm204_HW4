...
intecept_d <- lm_d$coefficients[1]
```
...
emissions_A <- seq(0,180,10)

#make the demand function for a 

demand_A_fn <- function(x){
  slope_a*(180 - x) + intercept_a #180 is the x intercept, has the same slope because it is just the inverse 
}

#evalute demand function at each value for emissions_A
demand_A <- demand_A_fn(emissions_A) 

#95.2 is the y intercept 

curve(demand_A_fn, from=0, to=180, xlab = "x", ylab = "y")

demand_df <- data.frame(emissions_A, demand_A)

#do the same for sector B
emissions_B <- seq(0,200,10)

demand_B_fn <- function(x){
  slope_b*(200 - x) + intercept_b
}

demand_B <- demand_B_fn(emissions_B)

curve(demand_B_fn, from=0, to = 200, xlab = "x", ylab = "y")

#demand_df <- data.frame(emissions_A, demand_A, emissions_B, demand_B)
```
